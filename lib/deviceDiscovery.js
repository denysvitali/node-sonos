...
var DeviceDiscovery = function DeviceDiscovery (options) {
  ...
  var sendDiscover = function () {
    ...
    self.socket.setMulticastInterface(self.localAddresses[self.interfaceAddrIndex])
    ...
    self.interfaceAddrIndex++
    self.interfaceAddrIndex = self.interfaceAddrIndex % self.localAddresses.length
  }
  ...
  this.socket.bind(options, function () {
    ...
    Object.keys(netif).forEach(i => {
      let el = netif[i]
      for (let eli in el) {
        console.log(el[eli])
        if (el[eli].family === 'IPv4' && el[eli].address !== '127.0.0.1') {
          self.localAddresses.push(el[eli].address)
        }
      }
    })
